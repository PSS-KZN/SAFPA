...
async function buildParlourDashboard(params) {
    ...
    const [members, policies, payments, funeralCases, branches] = await Promise.all([
        prisma_1.prisma.member.findMany({ where: memberWhere }),
        prisma_1.prisma.policy.findMany({ where: params.parlourId ? { parlourId: params.parlourId } : undefined }),
        prisma_1.prisma.paymentTransaction.findMany({ where: params.parlourId ? { parlourId: params.parlourId } : undefined }),
        prisma_1.prisma.funeralCase.findMany({ where: params.parlourId ? { parlourId: params.parlourId } : undefined }),
        prisma_1.prisma.branch.findMany({ where: params.parlourId ? { parlourId: params.parlourId } : undefined }),
    ]);
...
exports.reportsRouter.get('/dashboard', async (req, res) => {
...
exports.reportsRouter.get('/dashboard/export', async (req, res) => {
    const parlourId = typeof req.query.parlourId === 'string' ? req.query.parlourId : undefined;
    const branchId = typeof req.query.branchId === 'string' ? req.query.branchId : undefined;
    const startDate = typeof req.query.startDate === 'string' ? req.query.startDate : undefined;
    const endDate = typeof req.query.endDate === 'string' ? req.query.endDate : undefined;
    ...
    try {
        await (0, subscription_1.assertReportExportAllowed)(parlourId);
    }
    catch (error) {
