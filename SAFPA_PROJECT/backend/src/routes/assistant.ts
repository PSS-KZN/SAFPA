...
const assistantRequestSchema = z.object({
  ...
  context: z.object({
    ...
    entityType: z.enum(['policy', 'member', 'funeral_case']).optional(),
    entityId: z.string().optional(),
    currentPath: z.string().optional(),
  }).optional(),
});

type Actor = NonNullable<Express.Request['actor']>;

function normalizePrompt(value: string): string {
  return value.toLowerCase().replace(/[^a-z0-9\s]/g, ' ').replace(/\s+/g, ' ').trim();
}

function roleCapabilitySummary(role: string): string {
...
function policyStatusMeaning(status: string): string {
...
function allowedPolicyTransitions(policy: { status: string; allowedStatusTransitions: unknown }): string[] {
  if (policy.allowedStatusTransitions && typeof policy.allowedStatusTransitions === 'object' && !Array.isArray(policy.allowedStatusTransitions)) {
    const allowed = (policy.allowedStatusTransitions as Record<string, unknown>)[policy.status];
...
function countOpenTaskItems(value: unknown): number {
...
async function visibleMemberIds(actor: Actor): Promise<string[]> {
...
async function visiblePolicies(actor: Actor) {
...
async function visibleDocuments(actor: Actor) {
  if (actor.role === 'safpa_admin') {
...
async function visibleFuneralCases(actor: Actor) {
  if (actor.role === 'safpa_admin') {
    return prisma.funeralCase.findMany({ orderBy: { createdAt: 'desc' } });
  }
  ...
  if (actor.role === 'policyholder_customer') {
    ...
    return prisma.funeralCase.findMany({
...
async function visibleMembers(actor: Actor) {
  if (actor.role === 'safpa_admin') {
  ...
  if (actor.role === 'policyholder_customer') {
    ...
    return prisma.member.findMany({ where: { id: actor.memberId }, orderBy: { createdAt: 'desc' } });
  }
...
function findPolicyFromPrompt<T extends { id: string; policyNumber: string; memberId: string }>(
  policies: T[],
...
function findMemberFromPrompt<T extends { id: string; firstName: string; lastName: string }>(
  members: T[],
  prompt: string,
...
function findFuneralCaseFromPrompt<T extends { id: string; caseNumber: string; deceasedName: string }>(
  funeralCases: T[],
  prompt: string,
...
assistantRouter.post('/chat', async (req, res) => {
  ...
  const policy = findPolicyFromPrompt(
    policies,
    policyMemberLabels,
    prompt,
    parsed.data.context?.entityType === 'policy' ? parsed.data.context.entityId : undefined,
  );
  const member = findMemberFromPrompt(
    members,
    prompt,
    parsed.data.context?.entityType === 'member' ? parsed.data.context.entityId : undefined,
  );
  const funeralCase = findFuneralCaseFromPrompt(
    funeralCases,
    prompt,
    parsed.data.context?.entityType === 'funeral_case' ? parsed.data.context.entityId : undefined,
  );
  ...
  if (member && ((prompt.includes('member') || prompt.includes('customer') || prompt.includes('person') || prompt.includes('profile')) || (parsed.data.context?.entityType === 'member' && prompt.includes('explain')))) {
    ...
    const successfulPayments = await prisma.paymentTransaction.count({ where: { memberId: member.id, status: 'successful' } });
  ...
  if (policy && ((prompt.includes('policy') || prompt.includes('cover') || prompt.includes('benefit') || prompt.includes('status') || prompt.includes('reinstat')))) {
    ...
    const paymentCount = await prisma.paymentTransaction.count({ where: { policyId: policy.id } });
    return res.json({
  ...
  if (policy && (prompt.includes('payment') || prompt.includes('premium') || prompt.includes('due date') || prompt.includes('last payment'))) {
    const payments = await prisma.paymentTransaction.findMany({ where: { policyId: policy.id }, orderBy: { createdAt: 'desc' } });
