...
export default function TopBar() {
  ...
  return (
    <header className="h-16 bg-white border-b border-slate-200 flex items-center justify-between px-6 flex-shrink-0 relative z-20 shadow-sm" style={headerStyle}>
      {/* Search */}
      <div className="flex items-center gap-2 bg-slate-50 border border-slate-200 rounded-lg px-4 py-2 w-96 transition-all" style={isTenantBranded ? { boxShadow: `0 0 0 1px ${shellPrimary}20 inset` } : undefined}>
      ...
      <div className="flex items-center gap-6">
        {isTenantBranded && parlourBrand && (
          <div className="hidden items-center gap-3 rounded-xl border border-slate-200 bg-slate-50 px-4 py-2 lg:flex">
            ...
            <div>
              <div className="text-sm font-semibold text-slate-800">{parlourBrand.name}</div>
              <div className="text-xs text-slate-500">{parlourBrand.tagline || 'Tenant-branded workspace'}</div>
            </div>
          </div>
        )}
        ...
        <div className="flex items-center gap-3 pl-4 border-l border-slate-200">
          ...
          <div className="w-10 h-10 rounded-full text-white flex items-center justify-center text-sm font-bold shadow-sm border-2 border-white ring-2 ring-slate-100 cursor-pointer" style={{ backgroundColor: shellPrimary }}>
