...
export default function ReceiptView() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        setLoading(true);
        const records = await fetchPayments(currentUser.parlourId || 'p1');
  ...
  if (loading) {
    return <div className="text-center py-12 text-slate-500">Loading receipt...</div>;
  }

  if (!payment) {
    return <div className="text-center py-12 text-slate-500">Receipt not found</div>;
  }
