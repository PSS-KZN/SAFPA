...
import {
  bulkImportPoliciesFile,
  getPolicyBulkImportErrorFileUrl,
  type PolicyBulkImportResult,
} from '../../services/policiesApi';

export default function BulkImportPolicies() {
  const { currentUser } = useRole();
  const [file, setFile] = useState<File | null>(null);
  const [submitting, setSubmitting] = useState(false);
  const [result, setResult] = useState<PolicyBulkImportResult | null>(null);
  ...
  const importRows = async () => {
    ...
    try {
      setSubmitting(true);
      setMessage(null);
      const response = await bulkImportPoliciesFile(currentUser.parlourId || 'p1', file);
      setResult(response);
      setMessage(`Imported ${response.createdCount}/${response.totalRows} rows.`);
    } catch (error) {
      setResult(null);
      setMessage(error instanceof Error ? error.message : 'Failed to import policy rows');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <div>
      ...
      <h1 className="text-2xl font-bold mb-6">Bulk Import Policies</h1>
      ...
      <div className="bg-white rounded-xl p-8 shadow-sm border border-slate-200 max-w-3xl">
        ...
        <div className="mt-4 text-xs text-slate-400">
          <p>Required columns: memberId, productId, billingFrequency, startDate, nextDueDate</p>
          <p>Optional columns: policyNumber, productName, status, premiumAmount, coverAmount, arrearsAmount, lastPaymentDate</p>
        </div>
        ...
        {result && (
          <div className="mt-4 rounded-lg border border-slate-200 p-4 text-sm">
            ...
            {result.errorFileToken && (
              <a
                href={getPolicyBulkImportErrorFileUrl(result.errorFileToken)}
                target="_blank"
                rel="noreferrer"
                className="mt-2 inline-flex items-center gap-1 text-red-600 hover:text-red-800"
              >
