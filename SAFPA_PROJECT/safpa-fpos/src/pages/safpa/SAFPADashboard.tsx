...
import { safpaDashboardStats } from '../../data/dashboardStats';
...
export default function SAFPADashboard() {
  const stats = safpaDashboardStats;
  const cards = [
    ...
    { label: 'Collection Rate', value: `${stats.collectionRate}%`, sub: `R${(stats.premiumsCollectedThisMonth / 100).toLocaleString()}k collected`, icon: <Wallet size={24} className="text-white" />, gradient: 'from-amber-500 to-amber-600' },
    { label: 'Premiums Due', value: `R${(stats.premiumsDueThisMonth / 100).toLocaleString()}k`, sub: 'This month', icon: <TrendingUp size={24} className="text-white" />, gradient: 'from-slate-700 to-slate-800' },
    { label: 'Total Arrears', value: `R${(stats.totalArrears / 100).toLocaleString()}k`, sub: `${stats.openFuneralCases} open cases`, icon: <AlertTriangle size={24} className="text-white" />, gradient: 'from-red-600 to-red-800' },
  ];
